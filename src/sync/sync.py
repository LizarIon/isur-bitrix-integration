...
from src.config import BITRIX_WEBHOOK, GROUP_ID, ISUR_WORK_PARTICIPANTS_URL
from src.logger import log_info, log_warning, log_error
from src.utils.db_utils import (
    load_links, save_link, load_task_data, save_task_data,
    load_closed_tasks, save_closed_task, remove_link, init_db,
    save_completed_list
)
...
def run_pass4_completed(links):
    ...
    if completed_list:
        save_completed_list(completed_list)
        log_info(f"НАЙДЕНО ЗАВЕРШЁННЫХ ЗАДАЧ: {len(completed_list)}")
        log_info("Результат сохранён в БД (таблица completed_tasks)")
    else:
        ...
        try:
            ...
            if info["status"] == "5" or info["closed_date"]:
                ...
                if close_isur_task(isur_id):
                    ...
                    save_closed_task(isur_id, datetime.now().isoformat(), title)
                    closed_count += 1
                else:
